...
try {
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("UPDATE exam_answers SET answer = ?, updated_at = NOW() WHERE id = ?");
        $stmt->bind_param("si", $answer, $answer_id);
    } else {
        // Insérer une nouvelle réponse
        $stmt = $conn->prepare("INSERT INTO exam_answers (session_id, question_id, student_id, answer, created_at) 
                                VALUES (?, ?, ?, ?, NOW())");
